...
```python
c.NativeAuthenticator.email_server = {'url': 'smtp.gmail.com', 'usr': 'myself', 'pwd': 'mypassword'}
```

If you do not specify a `email_server`, it will attempt to use `localhost` without authentication.
