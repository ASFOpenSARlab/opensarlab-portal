...
class EmailPasswordResetHandler(LocalBase):
    async def get(self, slug):
        """Called on GET requests. The slug is given in the URL after /confirm-password/.
        ...
        message = "Invalid URL. Try resetting your password again. Otherwise, contact the administrator."

        if (
            self.authenticator.enable_forget_password
            and self.authenticator.ask_email_on_signup
        ):
            try:
                data = EmailPasswordResetHandler.validate_slug(
                    slug, self.authenticator.secret_key
                )
                slug_validation_successful = True
            except ValueError as e:
                self.log.error(f"Password reset slug validation failed. {e}")
            except Exception as e:
                self.log.error(f"Unknown error in validating reset password slug. {e}")

        if slug_validation_successful:
            username = data["username"]
            self.log.warning(
                f"Password reset confirmation validated for username '{username}': {slug}"
            )
            new_password = data["password"]
            success = self.authenticator.change_password(username, new_password)

            if success:
                message = f"The password for {username} has been changed successfully"
                ...
                message = f"Something went wrong with the password reset. The password for {username} was not changed"
